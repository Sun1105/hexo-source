---
title: Json格式的转化以及项目匹配
category: VBA
tags: [TOOL]
date: 2025-12-18T09:09:09.686Z
---
...
import javax.swing.*; // 导入Swing库用于GUI开发
import java.awt.*; // 导入AWT库用于布局
import java.awt.event.ActionEvent; // 导入AWT事件类（处理按钮事件）
import java.awt.event.ActionListener; // 导入AWT事件监听器接口
import java.io.*; // 导入IO库用于文件读写
import java.util.prefs.Preferences; // 导入Preferences类用于存储用户偏好设置
...
3，ImportKeyValueFromTxt（读取java程序生成的文件内容）
